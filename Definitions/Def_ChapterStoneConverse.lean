...
open BookProof.ChapterContinuityUnitaryInfinite (L2Z LinfZ summable_normSq norm_sq_eq_tsum)
...
noncomputable def mulOp (f : ℤ → ℝ) : mulDomain f →ₗ[ℂ] L2Z where
  ...
  map_add' a b := by
    ext k
    simp [mul_add]
    exact (Pi.add_apply (fun k => (f k : ℂ) * ((a : ℤ → ℂ) k))
      (fun k => (f k : ℂ) * ((b : ℤ → ℂ) k)) k).symm
  map_smul' c a := by
    ext k
...
theorem memℓp_two_of_summable {g : ℤ → ℂ} (h : Summable fun k => ‖g k‖ ^ 2) : Memℓp g 2 := by
  apply memℓp_gen
  simpa [show (2 : ENNReal).toReal = ((2 : ℕ) : ℝ) from by norm_num, Real.rpow_natCast] using h
...
noncomputable def shiftLin (m : ℤ) : L2Z →ₗ[ℂ] L2Z where
  ...
  map_add' f g := by
    ext k
    exact (Pi.add_apply (fun k => (f : ℤ → ℂ) (k + m))
      (fun k => (g : ℤ → ℂ) (k + m)) k).symm
...
noncomputable def velocityLin (v : LinfZ) : L2Z →ₗ[ℂ] L2Z where
  ...
  map_add' f g := by
    ext k
    simp [mul_add]
    exact (Pi.add_apply (fun k => ((v : ℤ → ℝ) k : ℂ) * ((f : ℤ → ℂ) k))
      (fun k => ((v : ℤ → ℝ) k : ℂ) * ((g : ℤ → ℂ) k)) k).symm
  map_smul' c f := by
    ext k
...
theorem velocityLin_norm_le (v : LinfZ) (f : L2Z) : ‖velocityLin v f‖ ≤ ‖v‖ * ‖f‖ := by
  ...
  rw [show (2 : ENNReal).toReal = ((2 : ℕ) : ℝ) from by norm_num]
...
noncomputable def phaseLin (f : ℤ → ℝ) (t : ℝ) : L2Z →ₗ[ℂ] L2Z where
  ...
  map_add' a b := by
    ext k
    simp [mul_add]
    exact (Pi.add_apply (fun k => phase f t k * ((a : ℤ → ℂ) k))
      (fun k => phase f t k * ((b : ℤ → ℂ) k)) k).symm
  map_smul' c a := by
    ext k
    simp only [lp.coeFn_smul, Pi.smul_apply, smul_eq_mul, RingHom.id_apply]
    ring
...
theorem tendsto_phaseUnitary (f : ℤ → ℝ) (psi : L2Z) :
  ...
  have hsqrt : Filter.Tendsto
      ...
      (nhds 0) (nhds 0) := by
    have h := (Real.continuous_sqrt.tendsto 0).comp htsum
...
def IsSelfAdjointOn (D : Submodule ℂ H) (A : D →ₗ[ℂ] H) : Prop :=
  BookProof.ChapterUnitaryTransport.adjointDomain D A = (D : Set H)
...
theorem transport_adjointDomain (W : H ≃ₗᵢ[ℂ] K) (D : Submodule ℂ H) (A : D →ₗ[ℂ] H) :
    BookProof.ChapterUnitaryTransport.adjointDomain (transportDomain W D) (transportOp W D A) = W '' BookProof.ChapterUnitaryTransport.adjointDomain D A := by
  ext phi'
  constructor
  · rintro ⟨eta', h⟩
...
theorem tendsto_transportUnitary (W : H ≃ₗᵢ[ℂ] K) (U : ℝ → H ≃ₗᵢ[ℂ] H)
  ...
  have this := (W.continuous.tendsto (W.symm y)).comp (h (W.symm y))
...
theorem tendsto_slope_transportUnitary (W : H ≃ₗᵢ[ℂ] K) (D : Submodule ℂ H) (A : D →ₗ[ℂ] H)
  ...
  have hlim : Filter.Tendsto
      ...
      (nhdsWithin 0 {0}ᶜ) (nhds (W (Complex.I • A a))) := hW
  rw [map_smul] at hlim
...
theorem adjointDomain_mulOp (f : ℤ → ℝ) :
    BookProof.ChapterUnitaryTransport.adjointDomain (mulDomain f) (mulOp f) = adjointDomain f :=
  rfl
