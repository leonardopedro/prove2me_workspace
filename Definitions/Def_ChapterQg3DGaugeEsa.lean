...
def torsionP (m : Fin 64) : MvPolynomial (Fin 84) ℂ :=
  tortorsionOps Φ m = Φ.op (mulOp (torsionP m)) := rfl
...
def torsionVec (m : Fin 64) (i : Fin 84) : ℝ :=
  (if i = torsionIdx1 m then·_sub_distrib, sum_single_X,
    sum_single_X]
  rfl
...
def qgFqQ (i j : Fin 84) : ℝ := (1 / 2)  Ful]
    congr 1
    ring
...
def qgSignedPoly (kappa : Fin 84 → ℝ) :
  ...
  ((1 / 2 : ℝ) : ℂ) •
    ((∑ j : Fin 84, ((kappa j : ℝ) : ℂ) • (pmom j).comp (pmom j))
      + ∑ m : Fin 64P  [nset.sum_congr rfl fun m _ => ?_
    rw [mul_assoc]
...
theorem qgSigned_eq_fqOp (kappa : Fin 84 → ℝ) :
  ...
  rw [fqOp, LinearMap.comp_apply, Submodule.subtype_apply, coreOp_coe,
    ← qgSignedPoly_eq_fqPoly kappa, pgLp_qgSigOp:=Reigned_essentiallySelfAdjointOn_core qgKappaElliptic
