#!/usr/bin/env python3
"""Build def bundles in topological order with streaming output."""
...
for i, db in enumerate(order):
    ...
    cmd = [BUILD_SCRIPT, db]
    t0 = time.time()
    print(f"\n[{i+1}/{total}] BUILDING: {db} ...", flush=True)
    result = subprocess.run(cmd, timeout=600)
    elapsed = time.time() - t0

    if result.returncode == 0:
        built_count += 1
        print(f"[{i+1}/{total}] OK ({elapsed:.0f}s): {db}", flush=True)
    else:
        output = f"  rc={result.returncode}"
        print(f"[{i+1}/{total}] FAIL ({elapsed:.0f}s): {db}{output}", flush=True)
        fail_count += 1
        failures.append((db, elapsed))

print(f"\n=== SUMMARY ===", flush=True)
print(f"Built: {built_count}, Skipped: {skip_count}, Failed: {fail_count}", flush=True)
for db, elapsed in failures:
    print(f"  FAILED ({elapsed:.0f}s): {db}", flush=True)
