#!/usr/bin/env python3
"""Compile def bundles in parallel with v4.33.1."""
...
def compile_one(def_name):
    ...
    try:
        ...
        if errors:
            return (def_name, False, "; ".join(errors[:3]))
        return (def_name, True, "")
    except subprocess.TimeoutExpired:
        return (def_name, False, "TIMEOUT")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(compile_one, dn): dn for dn in order}
        for future in as_completed(futures):
            def_name, success, err = future.result()
            if success:
                ok += 1
                if (ok + fail) % 10 == 0:
                    print("Progress: " + str(ok + fail) + "/" + str(len(order)) + " (" + str(ok) + " ok, " + str(fail) + " fail)", flush=True)
            else:
                fail += 1
